    ...
    <>
      ...
      >
        <div className={`flex items-center gap-1.5 ${isLeft ? 'flex-row-reverse' : ''}`}>
          <button onClick={data.onToggle} className="flex-shrink-0 transition-transform hover:scale-110">
            {done
              ? <CheckCircle2 className="w-3.5 h-3.5" style={{ color }} />
              : <Circle className="w-3.5 h-3.5 text-gray-300" />
            }
          </button>
          <span className={`leading-snug ${isLeft ? 'text-right' : 'text-left'} ${done ? 'font-medium text-gray-800' : 'text-gray-400'}`}>
            {data.label}
          </span>
        </div>
      </div>
